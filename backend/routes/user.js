    ...
    if (!err) {
      if (results.length <= 0) {
        ...
        var mailOptions = {
          ...
          to: results[0].email,
          subject: "Password by JoyBoy Companies",
          html:
            "<p><br> Your login details for JoyBoy Companies system</br> <br><b>Email: </b>" +
            results[0].email +
            "<br><b>Password: </b>" +
            results[0].password +
  ...
  connection.query(query, [email, user.oldPassword],(err, results) => {
    ...
    if(!err){
      if(results.length <= 0){
        ...
        return res.status(400).json({message: "Something wnt Wrong try again later"});
      }
